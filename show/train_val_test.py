...
def main():
    random.seed()
    source_file = '/DB/rhome/qyzheng/Desktop/qyzheng/source/renji_data/process/period/all_sizes_periods.csv'
    target_dir = '/DB/rhome/qyzheng/Desktop/qyzheng/source/renji_data/process/period/'
    test_percent = 0.2
    num_folds = 4
    ...
    save_slices_csv(test_slices, os.path.join(target_dir, 'period_test.csv'))
    ...
    for i, val in enumerate(cv):
        ...
        save_slices_csv(val_slices, os.path.join(target_dir, '{}_period_val.csv'.format(i)))
        save_slices_csv(train_slices, os.path.join(target_dir, '{}_period_train.csv'.format(i)))
