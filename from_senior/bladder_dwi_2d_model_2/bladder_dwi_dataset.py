...
class BladderDwiDataset(torch.utils.data.Dataset):
    def __init__(self, csv_file, preprocessor=None, augmentation=None, to_tensor=None, caching_data=False,
        ...
        self.label = samples[:, 9].astype(np.int)
        self.cancer_bboxes = samples[:, 3:6]
        self.sizes = samples[:, 6:9]
    ...
    def get_weights(self):
        ...
        w[2] = 1.0 / np.sum(self.label == 2)
        w[3] = 1.0 / np.sum(self.label == 3)
        w[4] = 1.0 / np.sum(self.label == 4)
        w[5] = 1.0 / np.sum(self.label == 5)
        w[6] = 1.0 / np.sum(self.label == 6)
        weights = []
        for l in self.label:
            weights.append(w[l])
        return weights
...
class MyPreprocessor(object):
    ...
    def __call__(self, sample):
        ...
        label = sample['label']
        label = sklearn.preprocessing.label_binarize([label], np.arange(7))
...
def test():
    ...
    dataset = BladderDwiDataset('/DB/rhome/qyzheng/Desktop/qyzheng/source/renji_data/process/dwi_t2w_t2wfs/0_cv_train.csv', preprocessor,
                                augmentation, to_tensor, False, False)
    ...
    for i in range(1000):
        ...
        if asample['label'] == 1:
            ...
            plt.show()
            plt.savefig('/DB/rhome/qyzheng/Desktop/qyzheng/source/renji_data/process/sample/my_unet/sample_{}_{}.png'.format(i, asample['accession_number']))
