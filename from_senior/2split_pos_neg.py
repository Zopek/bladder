    ...
    if access_num in labeled_access_num:
        for series in os.listdir(os.path.join(data_path, access_num)):
            ...
            try:
                ...
                box_label_path = os.path.join(label_path, access_num, series, stacks[0])
            except:
                ...
                if len(labeled_series) == 0:
                    ...
                    box_label_path = os.path.join(label_path, access_num, labeled_series[0], stacks[0])
...
f = open('/DATA3_DB7/data/public/renji_data/splits_accession_detection/positive_slices.csv', 'wb')
writer = csv.writer(f)
for i in range(len(positive_slices)):
    writer.writerow(positive_slices[i])

f = open('/DATA3_DB7/data/public/renji_data/splits_accession_detection/negative_slices.csv', 'wb')
writer = csv.writer(f)
for i in range(len(negative_slices)):
    writer.writerow(negative_slices[i])

cPickle.dump(positive_slices, open('/DATA3_DB7/data/public/renji_data/splits_accession_detection/positive_slices.txt', 'wb'))
cPickle.dump(negative_slices, open('/DATA3_DB7/data/public/renji_data/splits_accession_detection/negative_slices.txt', 'wb'))
