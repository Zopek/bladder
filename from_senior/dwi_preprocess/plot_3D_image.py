...
class Multi3DArrayPlane:
    ...
    def add(self, array, title=None, vmin=None, vmax=None, cmap='nipy_spectral', fixed_window=True, number=None):
        if fixed_window:
            if vmin is None:
                vmin = array.min()
            if vmax is None:
        ...
        if number is None:
            number = len(self.ax_list) + 1
        ax = self.figure.add_subplot(self.rows, self.columns, number)
...
if __name__ == '__main__':
    ...
    for a in array_list:
        plane.add(transpose_and_flip(a))
    plane.ready()
    plt.show()
