...
def pred_layer(in_channels, out_channels, dropout_prob):
    if dropout_prob != 0.0:
        return nn.Sequential(nn.Dropout2d(p=dropout_prob, inplace=False),
                             nn.Conv2d(in_channels, in_channels, kernel_size=1), nn.ReLU(inplace=False),
                             nn.Dropout2d(p=dropout_prob, inplace=False),
                             nn.Conv2d(in_channels, out_channels, kernel_size=1))
    else:
        return nn.Sequential(nn.Conv2d(in_channels, in_channels, kernel_size=1), nn.ReLU(inplace=False),
                             nn.Conv2d(in_channels, out_channels, kernel_size=1))
...
class UNet(nn.Module):
    def __init__(self, num_class, concat_pred_list, num_shared_encoders, dropout_prob_list):
        ...
        self.en_0 = EncodeAndPredictLayer(5, 64, num_class, False, concat_pred_list[0], share_list[0], dropout_prob_list[0])
