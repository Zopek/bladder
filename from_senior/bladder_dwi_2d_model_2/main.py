...
def main():
    ...
    # input dirs
    dataset_csv_dir = '/DB/rhome/qyzheng/Desktop/qyzheng/source/renji_data/process/period'
    ...
    # output dirs
    model_weights_dir = '/DB/rhome/qyzheng/Desktop/qyzheng/PROGRAM/bladder/from_senior/bladder_dwi_2d_model_2/model_weights'
    log_dir = '/DB/rhome/qyzheng/Desktop/qyzheng/PROGRAM/bladder/from_senior/bladder_dwi_2d_model_2/log'
    ...
    if mode == 'train':
        phases['cv_train'] = os.path.join(dataset_csv_dir, '{}_period_train.csv'.format(cv_id))
        phases['cv_val'] = os.path.join(dataset_csv_dir, '{}_period_val.csv'.format(cv_id))
    phases['test'] = os.path.join(dataset_csv_dir, 'period_test.csv')
    dataloaders = dict()
    for phase in phases:
        ...
        preprocessor = MyPreprocessor(image_root_dir, cancer_bboxes_root_dir, new_height, new_width, True)
        to_tensor = ToTensor()
        if is_training:
    ...
    for epoch in range(num_step_one_epoches + num_step_two_epoches):
        for phase in phases:
            ...
            for step, data in enumerate(dataloaders[phase]):
                ...
                for cam in preds_tuple[:-1]:
                    if mil_pooling_type == 'max':
                        score = torch.nn.functional.adaptive_max_pool2d(cam, (1, 7)).view(-1, 7)
                    elif mil_pooling_type == 'avg':
                        score = torch.nn.functional.adaptive_avg_pool2d(cam, (1, 7)).view(-1, 7)
                    else:
                        raise Exception('Unknown mil_pooling_type')
                    score = torch.nn.functional.softmax(score)
                    # loss_cam = torch.nn.functional.cross_entropy(score, label)
                ...
                # label_array_list.append(sklearn.preprocessing.label_binarize(label.data.cpu().squeeze().numpy(), np.arange(7)))
            ...
            confusion_matrix = sklearn.metrics.confusion_matrix(np.where(label==1)[1], np.argmax(score, 1))
            roc_auc = sklearn.metrics.roc_auc_score(label, score, average='micro')
            print(
